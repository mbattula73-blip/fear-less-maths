"""
Fear Less Maths — LEVEL 9 (Fractions, Grade 5+)
Swapped into the Level 9 position (was Level 6) on 2026-07-20: this is
the same v3 content, unchanged except for its sublevel codes -- Factors/
Multiples/HCF/LCM now sits at Level 6 instead, since HCF/LCM is a
genuine prerequisite for simplifying fractions and finding a common
denominator.
...
# ───────────────────────── Dispatcher (REPLACES original Level 9) ─────────────────────────
def _wrap(fn):
    return {1: lambda: fn(1), 2: lambda: fn(2), 3: lambda: fn(3), 4: lambda: fn(4)}


LEVEL9_DISPATCH = {
    "9A": _wrap(_A_s), "9B": _wrap(_B_s), "9C": _wrap(_C_s), "9CUM1": _wrap(_CUM1_s),
    "9D": _wrap(_D_s), "9E": _wrap(_E_s), "9F": _wrap(_F_s), "9CUM2": _wrap(_CUM2_s),
    "9G": _wrap(_G_s), "9H": _wrap(_H_s), "9I": _wrap(_I_s), "9CUM3": _wrap(_CUM3_s),
    "9J": _wrap(_J_s), "9REV": _wrap(_REV_s),
}
