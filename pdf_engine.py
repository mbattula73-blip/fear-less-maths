...
def _precise_diag_h(dtype, params):
    ...
    try:
        if compact_matching or graph_box or extra_compact or ultra_compact or circle_quad_box:
            ...
            from svglib.svglib import svg2rlg
            from io import StringIO
            svg_str = generate_svg_diagram(dtype, params)
            if not svg_str:
                return 34*mm
            d = svg2rlg(StringIO(svg_str))
...
def _est(item, cw=None):
    ...
    big_diag = item.get("diagram_type") in ("base10_blocks", "compare_blocks",
                                             ...
                                             "circle_sector", "circle_ring", "right_triangle_trig", "height_distance",
                                             "ap_sequence", "data_bar_chart", "probability_bag")
