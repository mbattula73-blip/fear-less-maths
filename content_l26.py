"""
Fear Less Maths — Question Content: Level 26 (Number Systems)
CBSE Class 9 Number Systems chapter, given its own full 14-sublevel
treatment (previously squeezed into Level 13 / not covered at all).
...
def _L26A_s(sheet):
    ...
    items = [
        cb("Natural, Whole & Integer Numbers", [
            ...
            "Every Natural number is Whole. Every Whole number is an Integer. N \u2286 W \u2286 Z.",
        ], "-4 is an Integer only. 0 is Whole (and Integer) but not Natural. 7 is all three."),
        cb("Closure — why each set had to be invented", [
            "A set is CLOSED under an operation if the result always stays in that set.",
            "N is NOT closed under subtraction: 3 - 5 = -2, which isn't a Natural number.",
            "Z IS closed under subtraction -- that's exactly why negative numbers (and Z) exist.",
        ], "Test: is W closed under subtraction? 2 - 5 = -3 -- not in W. So no."),
    ]
    for _ in range(4):
...
def _L26B_s(sheet):
    ...
    items = [
        cb("Rational Numbers (Q)", [
            ...
            "Every integer is rational: 5 = 5/1. Every terminating or repeating decimal is rational.",
            "Two fractions are EQUIVALENT (the same rational number) if they simplify to the same p/q.",
...
def _L26C_s(sheet):
    ...
    items = [
        cb("Predicting a Decimal Expansion", [
            ...
            "Only 2s and/or 5s -> the decimal TERMINATES (stops).",
            "Any other prime factor (3, 7, 11, ...) -> the decimal RECURS (repeats forever).",
...
def _L26CUM1_s(sheet):
    ...
    items = [
        cb("Converting a Recurring Decimal to a Fraction", [
            "Let x = the recurring decimal. Multiply by 10^(period length) to shift one full repeat left of the point.",
            "Subtract the original x -- the repeating parts cancel, leaving a whole number equation.",
            "Solve for x -- that IS the fraction (simplify if needed).",
...
def _L26D_s(sheet):
    ...
    items = [
        cb("Irrational Numbers", [
            "An irrational number CANNOT be written as p/q -- its decimal expansion never terminates and never repeats.",
            "\u221an is irrational whenever n is NOT a perfect square. \u221a4=2 is rational; \u221a5 is irrational.",
            "\u03c0 is irrational. 22/7 is just a RATIONAL approximation of \u03c0, not \u03c0 itself.",
...
def _L26E_s(sheet):
    ...
    items = [
        cb("The Real Number Line", [
            "Every point on the number line corresponds to exactly one real number -- rational or irrational.",
            "To locate an irrational like \u221a2, find which two consecutive integers it's between, then zoom in decimal place by decimal place -- SUCCESSIVE MAGNIFICATION.",
        ], "\u221a2 \u2248 1.414: it's between 1 and 2, then between 1.4 and 1.5, then between 1.41 and 1.42..."),
    ]
    for _ in range(6):
        n = random.choice(non_perfect)
        r = math.isqrt(n)
...
def _L26CUM2_s(sheet):
    ...
    items = [
        cb("Review: Rationals, Decimals, Irrationals, the Real Line", [
            "Rational = p/q form. Terminating or recurring decimal.",
            "Irrational = never terminates, never repeats. \u221an is irrational unless n is a perfect square.",
            "Together, rationals + irrationals make up the REAL numbers -- every point on the number line.",
        ], ""),
    ]
    for _ in range(5):
...
def _L26CUM3_s(sheet):
    ...
    items = [
        cb("Review: Surds & Rationalising", [
            ...
            "Rationalise a single-term surd denominator by multiplying by that same surd.",
...
def _L26J_s(sheet):
    ...
    items = [
        cb("Number Systems in the Real World", [
            ...
            "Recurring decimals show up constantly in real measurement and money contexts.",
            "Classifying a number correctly (rational vs irrational) matters for how precisely you can measure or represent it.",
...
def _L26REV_s(sheet):
    ...
    items = [
        cb("Level 26 Revision — Number Systems", [
            "N \u2286 W \u2286 Z \u2286 Q \u2286 R.  Irrationals fill the rest of R alongside Q.",
            "Denominator only 2s/5s -> terminates. Otherwise -> recurs.",
            "Simplify surds by pulling out perfect-square factors; rationalise using the surd itself (single term) or its conjugate (binomial).",
        ], ""),
    ]
    for _ in range(3):
...
DISPATCH_L26 = {
    "26A": {1: lambda: _L26A_s(1), 2: lambda: _L26A_s(2), 3: lambda: _L26A_s(3), 4: lambda: _L26A_s(4)},
    "26B": {1: lambda: _L26B_s(1), 2: lambda: _L26B_s(2), 3: lambda: _L26B_s(3), 4: lambda: _L26B_s(4)},
    "26C": {1: lambda: _L26C_s(1), 2: lambda: _L26C_s(2), 3: lambda: _L26C_s(3), 4: lambda: _L26C_s(4)},
    "26CUM1": {1: lambda: _L26CUM1_s(1), 2: lambda: _L26CUM1_s(2), 3: lambda: _L26CUM1_s(3), 4: lambda: _L26CUM1_s(4)},
    "26D": {1: lambda: _L26D_s(1), 2: lambda: _L26D_s(2), 3: lambda: _L26D_s(3), 4: lambda: _L26D_s(4)},
    "26E": {1: lambda: _L26E_s(1), 2: lambda: _L26E_s(2), 3: lambda: _L26E_s(3), 4: lambda: _L26E_s(4)},
    "26CUM2": {1: lambda: _L26CUM2_s(1), 2: lambda: _L26CUM2_s(2), 3: lambda: _L26CUM2_s(3), 4: lambda: _L26CUM2_s(4)},
    "26F": {1: lambda: _L26F_s(1), 2: lambda: _L26F_s(2), 3: lambda: _L26F_s(3), 4: lambda: _L26F_s(4)},
    "26G": {1: lambda: _L26G_s(1), 2: lambda: _L26G_s(2), 3: lambda: _L26G_s(3), 4: lambda: _L26G_s(4)},
    "26H": {1: lambda: _L26H_s(1), 2: lambda: _L26H_s(2), 3: lambda: _L26H_s(3), 4: lambda: _L26H_s(4)},
    "26CUM3": {1: lambda: _L26CUM3_s(1), 2: lambda: _L26CUM3_s(2), 3: lambda: _L26CUM3_s(3), 4: lambda: _L26CUM3_s(4)},
    "26I": {1: lambda: _L26I_s(1), 2: lambda: _L26I_s(2), 3: lambda: _L26I_s(3), 4: lambda: _L26I_s(4)},
    "26J": {1: lambda: _L26J_s(1), 2: lambda: _L26J_s(2), 3: lambda: _L26J_s(3), 4: lambda: _L26J_s(4)},
    "26REV": {1: lambda: _L26REV_s(1), 2: lambda: _L26REV_s(2), 3: lambda: _L26REV_s(3), 4: lambda: _L26REV_s(4)},
}
...
def _l26_wrap(fn):
    return lambda: _l26_visualize(fn())


DISPATCH_L26 = {
    sub: {sheet: _l26_wrap(fn) for sheet, fn in sheets.items()}
    for sub, sheets in DISPATCH_L26.items()
}
